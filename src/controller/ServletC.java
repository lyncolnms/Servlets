...
public class ServletC extends HttpServlet {
	...
	Scanner scanner;
	String cidades;
	ArrayList<String> cidadesC = new ArrayList<String>();
	...
	public void init(ServletConfig config) throws ServletException {
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if (cidades.startsWith("C")) {
					cidadesC.add(cidades);
				}
			}
	...
	protected void doGet(HttpServletRequest request,
		...
		out.println("<ul>");
		for (int i = 0; i < this.cidadesC.size(); i++) {
			out.println("<li>");
			out.println(this.cidadesC.get(i));
			out.println("</li>");
		}
		out.println("</ul>");
		out.println("</div>");
