...
public class ServletB extends HttpServlet {
	...
	Scanner scanner;
	String cidades;
	ArrayList<String> cidadesB = new ArrayList<String>();
	...
	public void init(ServletConfig config) throws ServletException {
		try {
			...
			while (scanner.hasNextLine()) {
				...
				if (cidades.startsWith("B")) {
					cidadesB.add(cidades);
				}
			}
	...
	protected void doGet(HttpServletRequest request,
		...
		out.println("<ul>");
		for (int i = 0; i < this.cidadesB.size(); i++) {
			out.println("<li>");
			out.println(this.cidadesB.get(i));
			out.println("</li>");
		}
		out.println("</ul>");
		out.println("</div>");
